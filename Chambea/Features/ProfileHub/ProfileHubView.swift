    ...
    private var hubActions: some View {
        VStack(spacing: 12) {
            hubRow(
                ...
                title: String(localized: "profile.edit"),
                destination: AnyView(ProfileEditorView(profile: $profile) { Task { await saveProfile() } })
            )
            hubRow(
                icon: "folder.fill",
            ...
            hubRow(
                icon: "link",
                title: String(localized: "profile.portfolio"),
                destination: AnyView(PortfolioEditorView(profile: $profile) { Task { await saveProfile() } })
            )
        }
    }
